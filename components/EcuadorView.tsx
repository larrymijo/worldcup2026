          ...
          <div className="flex flex-col gap-8 lg:flex-row lg:items-center lg:justify-between">
            <div className="max-w-xl">
              <span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-muted">
                <span className="float-slow text-base">🇪🇨</span> Group E · La Tri
              </span>
              ...
              <p className="mt-4 max-w-md text-sm leading-relaxed text-muted sm:text-base">
                Follow every Ecuador fixture — kick-off times shown in your own
                timezone{tz ? ` (${tz})` : ""}. Counting down to the next match below.
              </p>
            </div>
            ...
            <div className="w-full max-w-md lg:w-[26rem]">
              {nextMatch ? <NextMatchPanel match={nextMatch} now={now} /> : (
                <div className="glass rounded-2xl p-6 text-center text-muted">
                  {loading ? "Loading Ecuador's schedule…" : "Schedule unavailable."}
                </div>
              )}
            </div>
          </div>
  ...
  return (
    <div className="glass rounded-3xl p-5 sm:p-6">
      ...
      <div className="flex items-center justify-center gap-4">
        ...
        <span className="font-display text-sm text-muted">{venue ? "vs" : "@"}</span>
        <div className="flex flex-col items-center gap-2">
      ...
      <div className="mt-5 rounded-2xl border border-white/5 bg-night-2/60 p-4 text-center">
        <p className="text-sm font-semibold text-ink">{formatLocalDateLong(match.kickoff)}</p>
        <p className="text-xs text-muted">
          {formatLocalTime(match.kickoff)} · {match.venue}, {match.city}
        </p>
      </div>
      ...
      <div className="mt-5 flex justify-center">
        {isLiveOrFinished ? (
          <span className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm font-semibold text-muted">
            {status === "live" ? "⚽ Match in progress" : "Full time"}
          </span>
        ) : (
